# main.py (final, ohne Nachrichtenboxen)
...
class BestandInputDialog(QDialog):
    ...
    def check_line_count(self):
        ...
        if line_count >= self.required_lines:
            final_content = "\n".join(lines[:self.required_lines])
            self.text_edit.blockSignals(True)
            self.text_edit.setPlainText(final_content)
...
class MainWindow(QMainWindow):
    ...
    def load_csv_data(self, file_path: str) -> pd.DataFrame | None:
        try:
            ...
            # Optionale Fehlerbehandlung im Terminal, statt Pop-up
    ...
    def process_tier_ids(self, ids: list[str], df: pd.DataFrame) -> list[dict | None]:
        processed_data = []
        for tier_id in ids:
            if not tier_id or tier_id.lower() == 'keine kuh':
            ...
            if not match.empty:
                ...
                data_dict = {
                    'id': tier_info.get('Ohrmarke-Name'),
    ...
    def populate_gruppenboxen(self):
        ...
        for i in range(num_boxes):
            ...
            tiere_in_box = [
                self.gruppenboxen_processed_data[j] for j in range(start_index, end_index)
                if j < len(self.gruppenboxen_processed_data) and self.gruppenboxen_processed_data[j]
            ]
